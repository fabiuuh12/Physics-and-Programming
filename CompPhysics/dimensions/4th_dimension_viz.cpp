...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        studio::title("4D Tesseract Projection into 3D", studio::Style::Instrument);
        studio::help("Hold left mouse: orbit | wheel: zoom | +/- 4D speed | P pause | R reset");
        std::string hud = Hud(speed4D, paused);
        studio::readout(hud.c_str());
        studio::note("Blue/orange vertices indicate opposite w-hyperplanes");
        studio::fps();
