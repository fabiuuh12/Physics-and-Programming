...
The first learning agent is deliberately modest: a tabular Q-learning policy with discretized distance, radial and along-track geometry, relative velocity, closing speed, fuel remaining, and decision time.
...
The randomized scenarios are staged into easy, medium, and full difficulty ranges.
The greedy planner still gives a useful baseline, but the Q table by itself struggles to generalize across randomized starts.

The current policy evaluator therefore uses a one-decision projection guard.
If a learned action is unknown, low-confidence, unsafe, or clearly worse than the greedy action over the next decision interval, the evaluator falls back to the greedy planner.
This makes the policy safer and gives a stable benchmark, but it also means that the current Q-learning result should be read carefully: the guarded policy matches the planner, while the next goal is to make the learned table beat the planner without depending on the guard.

\begin{table}[h]
\centering
\begin{tabular}{lrrr}
\toprule
Difficulty & Random & Greedy & Guarded Q-learning \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Current 24-seed curriculum benchmark for random, greedy, and guarded tabular Q-learning policies.}
\end{table}
...
\begin{tabular}{lrrrr}
\toprule
Difficulty & Policy & Mean range & Best range & Mean speed \\
\midrule
Easy & Random & \(118.80~\mathrm{km}\) & \(23.55~\mathrm{km}\) & \(0.1286~\mathrm{km/s}\) \\
Easy & Greedy & \(4.91~\mathrm{km}\) & \(1.27~\mathrm{km}\) & \(0.0124~\mathrm{km/s}\) \\
Easy & Guarded Q-learning & \(4.91~\mathrm{km}\) & \(1.27~\mathrm{km}\) & \(0.0125~\mathrm{km/s}\) \\
Medium & Random & \(120.92~\mathrm{km}\) & \(9.75~\mathrm{km}\) & \(0.1289~\mathrm{km/s}\) \\
Medium & Greedy & \(19.28~\mathrm{km}\) & \(2.75~\mathrm{km}\) & \(0.0273~\mathrm{km/s}\) \\
Medium & Guarded Q-learning & \(19.28~\mathrm{km}\) & \(2.75~\mathrm{km}\) & \(0.0273~\mathrm{km/s}\) \\
Full & Random & \(152.22~\mathrm{km}\) & \(11.50~\mathrm{km}\) & \(0.1628~\mathrm{km/s}\) \\
Full & Greedy & \(44.89~\mathrm{km}\) & \(4.83~\mathrm{km}\) & \(0.0560~\mathrm{km/s}\) \\
Full & Guarded Q-learning & \(44.89~\mathrm{km}\) & \(4.83~\mathrm{km}\) & \(0.0560~\mathrm{km/s}\) \\
\bottomrule
\end{tabular}
\caption{Current mean and best final ranges from the same 24-seed benchmark.}
\end{table}

The easy benchmark improved from the earlier unguarded learned-policy result of \(1/24\) successes to \(20/24\) successes after the state representation, reward shaping, and projection guard were added.
Medium improved from \(2/24\) successes and \(52.35~\mathrm{km}\) mean final range to \(7/24\) successes and \(19.28~\mathrm{km}\) mean final range.
The full benchmark currently matches greedy at \(5/24\) successes.
This is progress, but the equality between greedy and guarded Q-learning also exposes the current limitation: the learned policy is not yet independently better than the baseline.
...
One successful Q-learning replay can look good, but the multi-seed curriculum evaluation tells the truth: the current learner is not robust enough to beat the planner yet.
The projection guard is useful engineering, but it is also a diagnostic.
When guarded Q-learning exactly matches greedy, the system is telling us that the learned action table still needs better generalization.
...
\begin{itemize}
    \item reduce the policy's dependence on greedy fallback and projection guards,
    \item improve the state bins for distance, radial position, along-track position, radial velocity, along-track velocity, closing speed, fuel, and time,
    \item adjust the reward so the agent learns controlled closing speed instead of only reducing distance,
    \item continue staged curriculum training across easy, medium, and full randomized scenarios,
    \item compare tabular Q-learning against a linear function approximator or another compact generalizing policy,
...
The randomized Q-learning result is the more important lesson: the guarded system can now match the greedy planner across the current benchmark, but it does not yet improve on that planner.
