...
- Rule-based intent parsing (`run`, `list files`, `open folder`, `stop process`, `time`, `date`, `search web`, `remember`, `recall`, `chat`)
...
- macOS UI mode (`--ui`) with a SceneKit-rendered 3D face
...
clang++ -std=c++20 -O2 -fobjc-arc -Iinclude \
  ...
  -framework AppKit -framework SceneKit -framework AVFoundation -framework Speech -framework Vision -framework CoreMedia -framework CoreVideo \
...
export ALICE_STT_CHUNK_TIMEOUT=8
export ALICE_TTS_RATE=185
export ALICE_VOICE=Samantha
# set to 0 to disable automatic natural-voice selection when ALICE_VOICE is empty
...
- For face tracking in UI mode, allow camera access for your terminal app in System Settings > Privacy & Security > Camera, then restart Terminal/iTerm.
